...
# echo $($PSQL "SELECT name FROM teams WHERE team_id IN (SELECT winner_id FROM games WHERE round='Eighth-Final' AND year=2014) ORDER BY name")

# echo -e "\nList of unique winning team names in the whole data set:"
# echo $($PSQL "SELECT DISTINCT name FROM teams JOIN games ON teams.team_Id = games.winner_id WHERE round='Final'")
